...
- `GET /booking-options` - fetch rooms/services + availability + deposit rate
...
2. Set a verify token (`WHATSAPP_VERIFY_TOKEN`) for webhook validation.
3. Point your WhatsApp webhook to:
   - `GET /whatsapp/webhook` for verification
   - `POST /whatsapp/webhook` for incoming message events
4. Without Twilio credentials, endpoint still returns a `previewReply` for testing logic.
